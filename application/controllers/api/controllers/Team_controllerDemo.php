...
class Team_controller extends REST_Controller
{
    ...
    public function updateTeam_put()
    {
        $received_Token = $this->input->request_headers();
        $token=  array_key_exists('auth',$received_Token);
        if($token == 1)
        ...
        if ($this->form_validation->run() == false) {
            ...
            $team_arr['id']=$jsonArray['id'];
            $checkid= $this->Team_model->checkId($jsonArray['id']);
          if(!empty($checkid) )
           {
            $team= $this->Team_model->update_team($jsonArray['id'], $team_arr);
            if($team)
             {
                $message = [
                    'status' => true,
    ...
    public function deleteTeam_delete()
    {
        $received_Token = $this->input->request_headers();
        $token=  array_key_exists('auth',$received_Token);
        if($token == 1)
       ...
       {  
        ...
        if(!empty($checkid))
        {
            $deleteTeam= $this->Team_model->delete_team($jsonArray['id']);
            if($deleteTeam)
            {
                $message = [
                    'status' => true,
